...
const FirebaseSync = (function() {
  ...
  const MAX_FILE_SIZE = 2 * 1024 * 1024; // 2 MB por archivo
  const DAILY_LIMIT = 2 * 1024 * 1024; // 2 MB por día (total entre ambos)
  ...
  // Obtener el uso diario desde localStorage
  function getDailyUsage() {
    const today = new Date().toISOString().split('T')[0];
    const key = `panquesito_daily_${today}`;
    const usage = parseInt(localStorage.getItem(key) || '0', 10);
    return usage;
  }

  // Registrar un archivo en el uso diario
  function addDailyUsage(fileSize) {
    const today = new Date().toISOString().split('T')[0];
    const key = `panquesito_daily_${today}`;
    const current = parseInt(localStorage.getItem(key) || '0', 10);
    localStorage.setItem(key, String(current + fileSize));
  }

  // Verificar el límite diario
  function checkDailyLimit(fileSize) {
    const today = new Date().toISOString().split('T')[0];
    const key = `panquesito_daily_${today}`;
    const current = parseInt(localStorage.getItem(key) || '0', 10);
    const totalIfAdded = current + fileSize;

    if (totalIfAdded > DAILY_LIMIT) {
      const remaining = Math.max(0, DAILY_LIMIT - current);
      return {
        ok: false,
        error: `Límite diario de 2 MB alcanzado. Usaste ${formatBytes(current)} de 2 MB hoy. Te quedan ${formatBytes(remaining)}.`
      };
    }
    return { ok: true };
  }
  ...
  function validateFileSize(fileSize) {
    if (fileSize > MAX_FILE_SIZE) {
      return {
        ok: false,
        error: `El archivo es demasiado grande (${formatBytes(fileSize)}). Límite: 2 MB por archivo`
      };
    }
    return { ok: true };
  }
