...
const Storage = (function() {
  ...
  async function compressLocalData() {
    try {
      const data = loadData();
      ...
      for (const post of posts) {
        if (post.fileBase64) {
          try {
            ...
            delete post.fileBase64;
            changed = true;
          } catch (e) {
            console.error('Error comprimiendo post', post.id, e);
          }
        }
      }
      if (changed) saveData(data);
      console.log('compressLocalData: ok');
      return true;
    } catch (e) {
      console.error('Error en compressLocalData:', e);
      return false;
    }
  }
  ...
  async function addPost(post) {
    ...
    // Si tiene archivo (blob), guardarlo en IndexedDB y no en localStorage
    ...
    // Si tiene base64, convertirlo a blob y guardarlo en IndexedDB

    // (evita llenar el localStorage con archivos de hasta 2 MB)
    ...
    if (post.fileBase64) {
      ...
      try {
        ...
        console.error('Error guardando archivo base64 en IndexedDB:', e);
