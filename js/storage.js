/* ============================================
   ...
   - localStorage para metadatos y textos
   - IndexedDB para archivos binarios grandes
   - Sincronización directa con GitHub (carpeta info)
   ============================================ */
...
const Storage = (function() {
  ...
  const GITHUB_DATA_PATH = 'info/datos.json';
  ...
  async function addPost(post) {
    ...
    // Sincronizar con GitHub (guardar directamente en el repo)
  ...
  // Lee y escribe directamente en info/datos.json del repositorio

  // Cargar datos desde GitHub (info/datos.json)
  async function loadFromGithub() {
    const config = getConfig();
    const { owner, repo, token } = config.github;

    if (!owner || !repo || !token) {
      return { ok: false, error: 'Configura GitHub primero en Ajustes' };
    }

    const apiBase = `https://api.github.com/repos/${owner}/${repo}`;

    try {
      const getResp = await fetch(`${apiBase}/contents/${GITHUB_DATA_PATH}`, {
        headers: {
      ...
      if (getResp.ok) {
        ...
        const content = atob(remote.content);
        ...
        return { ok: false, error: 'info/datos.json no existe en el repositorio' };
      } else {
        const err = await getResp.json();
        throw new Error(getGithubErrorMessage(getResp.status, err));
      }
    } catch (e) {
  ...
  async function syncWithGithub() {
    ...
    try {
      ...
      // Intentar obtener los datos remotos desde info/datos.json
      let remoteData = null;
      let remoteSha = null;
      ...
      try {
        ...
        if (getResp.ok) {
          ...
          console.log('📁 info/datos.json no existe, se creará');
        } else {
          const err = await getResp.json();
          throw new Error(getGithubErrorMessage(getResp.status, err));
        }
      } catch (e) {
        if (e.message.includes('no existe') || e.message.includes('404')) {
          // No existe, se creará
        } else {
          throw e;
        }
      ...
      // Fusionar datos: combinar posts y chat (evitar duplicados por id)
      ...
      // Subir datos fusionados a info/datos.json
      ...
      const putResp = await fetch(`${apiBase}/contents/${GITHUB_DATA_PATH}`, {
        method: 'PUT',
        headers: {
          'Authorization': `token ${token}`,
          'Accept': 'application/vnd.github.v3+json'
      ...
      if (!putResp.ok) {
        ...
        throw new Error(getGithubErrorMessage(putResp.status, err));
      }
  ...
  // Función para obtener mensajes de error detallados de GitHub
  function getGithubErrorMessage(status, err) {
    const messages = {
      401: '❌ Token inválido o expirado. Crea un token nuevo en GitHub → Settings → Developer settings → Personal access tokens',
      403: '❌ Sin permisos. El token necesita el permiso "repo". O has excedido el límite de requests',
      404: '❌ Repositorio no encontrado. Verifica el nombre del usuario y del repositorio',
    ...
    if (err && err.message) {
      ...
      if (err.message.includes('Bad credentials')) return '❌ Token inválido o expirado';
      if (err.message.includes('Not Found')) return '❌ Repositorio no encontrado. Verifica usuario y nombre del repo';
      if (err.message.includes('Repository creation failed')) return '❌ No se pudo crear el repositorio';
      if (err.message.includes('already exists')) return '❌ El archivo ya existe';
