...
(function() {
      ...
      if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
          if (confirm(`¿Eliminar "${post.title || post.fileName}"? Esta acción no se puede deshacer.`)) {
            ...
            // Eliminar de GitHub
            if (GitHubDB.isConnected()) {
              await GitHubDB.deletePost(post.id);
            }
  ...
  async function sendChatMessage() {
    ...
    // Guardar en GitHub
    if (GitHubDB.isConnected()) {
      const result = await GitHubDB.addChatMessage(msg);
      if (!result.ok) {
        console.log('No se pudo guardar en GitHub:', result.error);
      }
    }
  ...
  async function submitFile() {
    ...
    try {
      ...
      // Guardar en GitHub
      if (GitHubDB.isConnected()) {
        fileSubmit.textContent = 'Subiendo a GitHub...';
        const result = await GitHubDB.addPost(post);
        if (result.ok) {
          showToast('🐙 Sincronizado con GitHub');
        } else {
          console.log('No se pudo subir a GitHub:', result.error);
        }
      }
  ...
  async function syncFromGithub(showmessage = false) {
    ...
    const result = await GitHubDB.loadData();
    if (result.ok) {
      ...
      if (showmessage) {
        showToast('⚠️ Error al sincronizar con GitHub');
      }
    }
  }
