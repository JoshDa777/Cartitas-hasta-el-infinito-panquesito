...
(function() {
  ...
  async function initApp() {
    // Cargar datos desde GitHub si está configurado
    const config = Storage.getConfig();
    if (config.github.owner && config.github.repo && config.github.token) {
      syncStatus.textContent = '🔄';
      syncStatus.classList.add('syncing');
      const result = await Storage.loadFromGithub();
      syncStatus.classList.remove('syncing');

      if (result.ok) {
        syncStatus.textContent = '✅';
        showToast('📡 Datos cargados desde GitHub');
      } else {
        syncStatus.textContent = '⚠️';
        console.log('No se pudieron cargar datos de GitHub:', result.error);
      }
    }
