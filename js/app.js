/* ============================================
   ...
   - Usa GitHub API como base de datos
...
(function() {
  ...
  // ====== BACKENDS HÍBRIDOS (GitHub + Firebase) ======
  // Ejecuta un método en TODOS los backends conectados
  async function cloudCall(method, ...args) {
    const results = { github: null, firebase: null, anyOk: false };

    if (typeof GitHubDB !== 'undefined' && GitHubDB.isConnected()) {
      try {
        results.github = await GitHubDB[method](...args);
      } catch (e) {
        results.github = { ok: false, error: e.message };
      }
    }

    if (typeof FirebaseDB !== 'undefined' && FirebaseDB.isConnected()) {
      try {
        results.firebase = await FirebaseDB[method](...args);
      } catch (e) {
        results.firebase = { ok: false, error: e.message };
      }
    }

    results.anyOk = !!(results.github && results.github.ok) || !!(results.firebase && results.firebase.ok);
    return results;
  }

  // ¿Hay al menos un backend (cloud) conectado?
  function cloudConnected() {
    const githubOk = typeof GitHubDB !== 'undefined' && GitHubDB.isConnected();
    const firebaseOk = typeof FirebaseDB !== 'undefined' && FirebaseDB.isConnected();
    return githubOk || firebaseOk;
  }
      ...
      if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
          if (confirm(`¿Eliminar "${post.title || post.fileName}"? Esta acción no se puede deshacer.`)) {
            ...
            // Eliminar del cloud (GitHub + Firebase)
            await cloudCall('deletePost', post.id);
  ...
  async function sendChatMessage() {
    ...
    // Guardar en el cloud (GitHub + Firebase)
    await cloudCall('addChatMessage', msg);
  ...
  async function submitFile() {
    ...
    if (file.size > 2 * 1024 * 1024) {
      showToast(`⚠️ El archivo es demasiado grande (${formatFileSize(file.size)}). Límite: 2 MB por archivo`);
      fileSubmit.disabled = false;
    ...
    const dailyKey = `panquesito_daily_${today}`;
    const currentDaily = parseInt(localStorage.getItem(dailyKey) || '0', 10);
    if (currentDaily + file.size > 2 * 1024 * 1024) {
      const remaining = Math.max(0, 2 * 1024 * 1024 - currentDaily);
      showToast(`⚠️ Límite diario de 2 MB alcanzado. Te quedan ${formatFileSize(remaining)}.`);
      fileSubmit.disabled = false;
      fileSubmit.textContent = 'Subir Archivo';
      return;
    }
    ...
    fileSubmit.textContent = 'Convirtiendo...';

    try {
      // Convertir archivo a base64
      const base64 = await GitHubDB.fileToBase64(file);
      ...
      // Guardar localmente (Storage.addPost mueve el base64 a IndexedDB)
      await Storage.addPost(post);
      showToast('✅ ¡Recuerdo guardado! 💖');

      // Registrar uso diario
      localStorage.setItem(dailyKey, String(currentDaily + file.size));

      // Guardar en el cloud: enviar una copia CON el base64
      if (cloudConnected()) {
        fileSubmit.textContent = 'Subiendo a la nube...';
        const postForCloud = JSON.parse(JSON.stringify(post));
        const result = await cloudCall('addPost', postForCloud);
        if (result.anyOk) {
          showToast('🐙 Sincronizado con la nube');
        } else {
          console.log('No se pudo subir al cloud:', result.github?.error || result.firebase?.error);
        }
      }
  ...
  async function submitWrite() {
    ...
    try {
      ...
      // Guardar en el cloud (GitHub + Firebase)
      if (cloudConnected()) {
        const result = await cloudCall('addPost', post);
        if (result.anyOk) {
          showToast('🐙 Sincronizado con la nube');
        } else {
          console.log('No se pudo subir al cloud:', result.github?.error || result.firebase?.error);
  ...
  async function getPostFileSource(postId, post) {
    // 1. Primero intentar localmente (IndexedDB)
    const fileData = await Storage.getFileFromDB(postId);
    if (fileData) {
      return { blob: fileData.blob };
    }

    // 2. Si el post tiene base64, convertirlo a blob
    if (post.fileBase64) {
      const blob = GitHubDB.base64ToBlob(post.fileBase64, post.fileType);
  ...
  async function downloadFile(postId) {
    ...
    try {
      const source = await getPostFileSource(postId, post);
      if (!source) {
  ...
  async function viewFile(postId) {
    ...
    try {
      ...
      switch (fileCategory.tipo) {
        ...
        case 'texto':
        case 'codigo':
          await source.blob.text().then(text => {
  ...
  // ====== SINCRONIZACIÓN CON EL CLOUD (GitHub + Firebase) ======
  async function syncFromGithub(showmessage = false) {
    const backends = [];
    if (typeof GitHubDB !== 'undefined' && GitHubDB.isConnected()) backends.push(GitHubDB);
    if (typeof FirebaseDB !== 'undefined' && FirebaseDB.isConnected()) backends.push(FirebaseDB);
    ...
    try {
      for (const backend of backends) {
        const result = await backend.loadData();
        if (!result.ok) {
          console.warn('Sincronizacion fallo en un backend:', result.error);
          continue;
        }
        anyOk = true;
        ...
        if (data.posts) {
          ...
          const newPosts = data.posts.filter(p => !localIds.has(p.id));

          for (const post of newPosts) {
            // Si el post trae base64, guardar el blob en IndexedDB y quitar el base64 del localStorage
            let postToSave = post;
            if (post.fileBase64) {
              try {
                const toBlob = (backend.base64ToBlob || GitHubDB.base64ToBlob);
                const blob = toBlob(post.fileBase64, post.fileType);
                await Storage.saveFileToDB(post.id, blob, post.fileName, post.fileType);
              } catch (e) {
                console.error('No se pudo guardar el blob del post:', e);
              }
              const { fileBase64, ...cleanPost } = post;
              postToSave = cleanPost;
            }

            await Storage.addPost(postToSave);
        ...
        if (data.chat) {
          ...
          const newMessages = data.chat.filter(m => !localIds.has(m.id));
      ...
      // SIEMPRE se quita el spinner, pase lo que pase
      syncStatus.classList.remove('syncing');
      syncStatus.textContent = anyOk ? '✅' : '⚠️';
      state.lastSync = new Date();

      if (showmessage) {
        showToast(anyOk ? '🐙 Sincronizado con la nube' : '⚠️ Error al sincronizar (revisa el token)');
      }
  ...
  syncStatus.addEventListener('click', () => syncFromGithub(true));
  ...
  function loadSettings() {
    ...
    if (typeof GitHubDB !== 'undefined' && GitHubDB.isConnected()) {
      githubStatus.textContent = '✅ GitHub conectado';
    } else if (config.github?.owner) {
      githubStatus.textContent = '⚠️ GitHub configurado pero no conectado';
    } else {
      githubStatus.textContent = 'ℹ️ GitHub no configurado';
    }

    // Estado de Firebase (si el SDK está cargado)
    const firebaseStatusEl = document.getElementById('firebase-status');
    if (firebaseStatusEl) {
      if (typeof FirebaseDB !== 'undefined' && FirebaseDB.isConnected()) {
        firebaseStatusEl.textContent = '✅ Firebase conectado';
      } else {
        firebaseStatusEl.textContent = '⚠️ Firebase no conectado (revisa la consola)';
      }
    }
  ...
  githubSave.addEventListener('click', async () => {
    ...
    githubSave.textContent = 'Conectando...';

    Storage.saveConfig({ github: githubConfig });

    const result = GitHubDB.init(githubConfig);

    githubSave.disabled = false;
    githubSave.textContent = 'Conectar GitHub';

    if (result.ok) {
      // Probar la conexion con una lectura real
      const test = await GitHubDB.loadData();
      if (test.ok) {
        githubStatus.textContent = '✅ ¡GitHub conectado correctamente!';
        showToast('🐙 GitHub conectado');
        await syncFromGithub(true);
      } else {
        githubStatus.textContent = `❌ El token no funciona: ${test.error}`;
        showToast('❌ Token de GitHub invalido o caducado');
        console.error('GitHub test fallo:', test.error);
      }
    } else {
  ...
  async function loadRemoteConfig() {
    try {
      ...
      if (resp.ok) {
        ...
        if (remoteData.config && remoteData.config.github) {
          const current = Storage.getConfig();
          const currentToken = (current.github && current.github.token) || '';

          if (currentToken) {
            // El usuario ya tiene token guardado en Ajustes/localStorage:
            // SOLO completar owner/repo si faltan, pero NUNCA pisar el token manual.
            const mergedConfig = {
              github: {
                owner: (current.github && current.github.owner) || remoteData.config.github.owner,
                repo: (current.github && current.github.repo) || remoteData.config.github.repo,
                token: currentToken
              },
            ...
            // Primera vez sin config manual: usar la del repo
            const mergedConfig = {
              github: remoteData.config.github,
              notifications: remoteData.config.notifications !== undefined
                ? remoteData.config.notifications
                : current.notifications
            };
  ...
  async function initApp() {
    // Cargar configuración desde info/datos.json (credenciales de GitHub)
    await loadRemoteConfig();

    // Saca los base64 del localStorage y los pasa a IndexedDB
    if (typeof Storage.compressLocalData === 'function') {
      await Storage.compressLocalData();
    }

    const config = Storage.getConfig();

    // Inicializar GitHub si está configurado
    if (config.github && config.github.owner) {
      syncStatus.textContent = '🔄';
      syncStatus.classList.add('syncing');

      const result = GitHubDB.init(config.github);

      if (result.ok) {
        await syncFromGithub(false);
      } else {
        syncStatus.textContent = '⚠️';
        console.log('Error al conectar GitHub:', result.error);
      }
    }

    // Inicializar Firebase (siempre intentamos conectar)
    if (typeof FirebaseDB !== 'undefined') {
      const fbResult = FirebaseDB.init();
      if (fbResult.ok) {
        console.log('Firebase conectado');
        await syncFromGithub(false);
      } else {
        console.log('Error al conectar Firebase:', fbResult.error);
      }
    }
