...
(function() {
  ...
  async function submitFile() {
    ...
    // IMPORTANTE: Conservar el blob para Firebase ANTES de Storage.addPost
    // (Storage.addPost elimina post.fileData del objeto para no guardarlo en localStorage)
    const fileBlob = file;
    ...
    try {
      // Subir a Firebase PRIMERO (Storage + Database) para que el archivo esté en la nube
      if (FirebaseSync.isConnected()) {
        fileSubmit.textContent = 'Subiendo a la nube...';
        // Crear una copia del post con el blob para Firebase
        const fbPost = { ...post, fileData: { blob: fileBlob, name: file.name, type: file.type || 'application/octet-stream' } };
        const fbResult = await FirebaseSync.savePost(fbPost);
        if (fbResult.ok) {
          // Actualizar el post local con la URL de Firebase
          post.fileUrl = fbPost.fileUrl;
          post.fileStoragePath = fbPost.fileStoragePath;
      ...
      // Guardar localmente (esto elimina fileData internamente)
  ...
  // Devuelve { blob } si se obtuvo localmente, o { url } para URL directa de Firebase
  async function getPostFileSource(postId, post) {
    ...
    // 2. Firebase Storage - intentar obtener URL directa fresca
    if (post.fileStoragePath || post.fileUrl) {
      showToast('☁️ Cargando desde Firebase...');
      const url = await FirebaseSync.getDownloadUrl(post.fileStoragePath || post.fileUrl);

      if (url) {
        // Intentar cachear localmente (puede fallar por CORS, usar URL directa entonces)
        try {
          ...
          console.log('CORS prevent caching, using direct URL:', e.message);
        }
        // Si fetch falla (CORS), el navegador igual puede cargar la URL directamente
        return { url };
      }
    }

    // 3. Respaldo: URL antigua
    if (post.fileUrl) {
      return { url: post.fileUrl };
    }
  ...
  async function downloadFile(postId) {
    ...
    try {
      ...
      // Si tenemos URL directa de Firebase (CORS), abrir en nueva pestaña para descargar
      if (source.url) {
        showToast('⬇️ Abriendo descarga desde Firebase...');
        const a = document.createElement('a');
        a.href = source.url;
        a.download = post.fileName || 'archivo';
        a.target = '_blank';
  ...
  async function viewFile(postId) {
    ...
    try {
      ...
      // Determinar la URL a usar: blob local o URL directa de Firebase
      let url = source.url || (source.blob ? URL.createObjectURL(source.blob) : null);
      if (!url) {
        showToast('❌ Archivo no encontrado');
        return;
      }
      ...
      switch (fileCategory.tipo) {
        ...
        case 'codigo':
          if (source.blob) {
            ...
            viewerHTML = `<iframe src="${url}" class="viewer-iframe"></iframe>`;
          }
          break;
        case 'pdf':
