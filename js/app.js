...
(function() {
  ...
  const githubPagesUrl = $('github-pages-url');
  ...
  async function syncToGithub(showmessage = false) {
    ...
    const result = await Storage.loadFromGithubPages();
    ...
    if (result.ok) {
      ...
      if (showmessage) {
        showToast('📡 Datos cargados desde GitHub Pages');
      }
    } else {
  ...
  function loadSettings() {
    const config = Storage.getConfig();
    githubPagesUrl.value = config.githubPagesUrl || '';
  ...
  githubSave.addEventListener('click', async () => {
    const url = githubPagesUrl.value.trim();

    if (!url) {
      showToast('⚠️ Pega la URL de GitHub Pages');
      return;
    }
    ...
    Storage.saveConfig({ githubPagesUrl: url });

    // Probar conexión cargando datos
    const result = await Storage.loadFromGithubPages();

    githubSave.disabled = false;
    githubSave.textContent = 'Guardar y Cargar Datos';

    if (result.ok) {
      githubStatus.textContent = '✅ ¡Datos cargados desde GitHub Pages!';
      showToast('✅ Datos cargados desde GitHub Pages');
      renderPosts();
      renderCalendar();
      renderChat();
  ...
  async function initApp() {
    // Cargar datos desde GitHub Pages si está configurado
    const config = Storage.getConfig();
    if (config.githubPagesUrl) {
      syncStatus.textContent = '🔄';
      syncStatus.classList.add('syncing');
      ...
      if (result.ok) {
        ...
        syncStatus.textContent = '⚠️';
        console.log('No se pudieron cargar datos de GitHub Pages:', result.error);
      }
    }
